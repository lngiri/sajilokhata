...
import { getCustomerProfile, submitCustomerEntry, updateCustomerProfile } from "@/app/actions/customer";
import {
...
export default function CustomerDashboardClient({ messages, locale }: Props) {
  ...
  const [voucherResizedBlob, setVoucherResizedBlob] = useState<Blob | null>(null);
            ...
            {customerName && (
              <p className="text-xs text-[var(--color-text-muted)] flex items-center gap-1">
                ...
                <button
                  ...
                  title={showFullPhone ? t("dashboard.hideNumber") : t("dashboard.showNumber")}
                >
                  {showFullPhone ? (
          ...
          <div className="w-full max-w-md bg-[var(--color-surface)] rounded-3xl p-6 animate-slide-up">
            ...
            <div className="flex gap-3 mt-6">
              ...
              <button
                onClick={async () => {
                  if (!editName.trim()) {
                    addToast(t("settings.nameEmpty") || "Name cannot be empty", "error");
                    return;
                  }
                  try {
                    const result = await updateCustomerProfile(customerPhone, { name: editName.trim() });
                    if (result.success) {
                      ...
                      addToast(result.error || t("toast.errorGeneric"), "error");
                    }
                  } catch {
            ...
            <div className="space-y-4">
              ...
              {voucherPreview && (
                <div className="relative">
                  ...
                  <button
                    onClick={() => { setVoucherPreview(null); setVoucherFile(null); setVoucherResizedBlob(null); }}
              ...
              <div className="flex gap-3 pt-2">
                ...
                <button
                  onClick={async () => {
                    if (!voucherResizedBlob || !voucherFile) return;
                    ...
                    try {
                      ...
                      const resizedFile = new File([voucherResizedBlob], voucherFile.name, { type: voucherResizedBlob.type });
                  ...
                  disabled={!voucherResizedBlob || voucherUploading}
                  className="flex-1 py-3.5 bg-[var(--color-primary-surface)] text-[var(--color-primary-foreground)] rounded-xl font-semibold active:scale-[0.98] transition-transform disabled:opacity-50 flex items-center justify-center gap-2"
                >
                  {voucherUploading ? (
                    <>
