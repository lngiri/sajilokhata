  ...
  useEffect(() => {
    ...
    (async () => {
      if (typeof window === "undefined") return;
            ...
            // 1. Skip session check if user just signed out — show welcome modal
      //    Also clear any stale last-session data so the user can enter a new number
...
// 2. Check for last signed-out session (quick re-login for dual-role)
      ...
      try {
        ...
        if (data.userId) {
          ...
          if (data.roles.length === 1) {
            const target = data.roles[0] === "merchant" ? "/merchant/dashboard" : `/${locale}/customer/dashboard`;
          ...
          window.location.replace(`/${locale}/select-role`);
          return;
        }
    ...
    if (addRoleTarget) {
      try {
        ...
        if (!otpResult.success) {
          setError(otpResult.error || t("errorFailedOtp"));
          setLoading(false);
          return;
        }
      } catch (e) {
        ...
        setError((e as Error)?.message || t("errorNetwork"));
        setLoading(false);
        return;
      }
      setStep("otp");
  ...
  const handlePinSubmit = async () => {
    ...
    try {
      ...
      if (!result.success) {
        if (!mountedRef.current) return;
        setError(result.error || t("errorIncorrectPin"));
        setPin(["", "", "", ""]);
  ...
  const handleSetPin = async () => {
    ...
    if (newPinStr.length < 4) { setError(t("errorPinLength")); return; }
    ...
    try {
      ...
      const target = info.userType === "customer" ? `/${locale}/customer/dashboard` : "/merchant/dashboard";
  ...
  const handleSkipPin = async () => {
    ...
    const target = info?.userType === "customer" ? `/${locale}/customer/dashboard`
      : info?.userType === "merchant" ? "/merchant/dashboard"
      : info?.userType === "both" ? `/${locale}/select-role`
      : "/merchant/dashboard";
  ...
  const handleOtpSubmit = async () => {
    ...
    try {
      ...
      if (!result.success) {
        if (!mountedRef.current) return;
        setError(result.error || t("errorInvalidOtp"));
        setLoading(false);
        return;
      }
      ...
      if (addRoleTarget) {
        ...
        if (!regResult.success) {
          if (!mountedRef.current) return;
          setError(regResult.error || t("errorCreateFailed"));
          setLoading(false);
          return;
        }
      ...
      if (hasPin) {
        const target = userType === "customer" ? `/${locale}/customer/dashboard` : "/merchant/dashboard";
  ...
  const handleForgotPhoneSubmit = async () => {
    ...
    try {
      ...
      if (!result.success) {
        setError(result.error || t("errorFailedOtp"));
        setLoading(false);
        return;
      }
      setStep("forgot_otp");
  ...
  const handleForgotOtpSubmit = async () => {
    ...
    try {
      ...
      if (!result.success) {
        if (!mountedRef.current) return;
        setError(result.error || t("errorVerificationFailed"));
        setLoading(false);
        return;
      }
  ...
  if (step === "loading") {
    return (
      <div className="min-h-dvh flex flex-col items-center justify-center px-6 py-12 bg-[var(--color-bg)]">
        ...
        <p className="mt-4 text-sm text-[var(--color-text-muted)]">{t("checkingSession")}</p>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-dvh flex flex-col items-center justify-center px-6 py-12 bg-[var(--color-bg)]">
      <div className="mb-8 text-center animate-fade-in">
        ...
        <h1 className="text-2xl font-extrabold text-[var(--color-text)]">{t("welcomeTitle")}</h1>
        <p className="text-sm text-[var(--color-text-muted)] mt-1">{t("welcomeSubtitle")}</p>
        {(step === "pin" || step === "set_pin") && userInfoRef.current?.userType && (
          <div className={`mt-3 inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold ${
            ...
            <span>
              {userInfoRef.current.userType === "both" ? `${t("merchant")} & ${t("customer")}`
                : userInfoRef.current.userType === "merchant" ? t("merchant")
                : t("customer")}
            </span>
          </div>
        )}
      ...
      {step === "pin" && (
        <div className="w-full max-w-xs space-y-6 animate-fade-in">
          {renderPinDots(pin, setPin, pinRefs, t("enterPin"), handlePinSubmit)}
      ...
      {step === "select_role" && (
        <div className="w-full max-w-xs space-y-6 animate-fade-in">
          {selectRoleMode === "register" && (
            <div>
              <label className="text-sm font-medium text-[var(--color-text)]">{t("setNameLabel")}</label>
              <input
                type="text"
                value={registerName}
          ...
          <div className="space-y-3">
            {availableRoles.includes("merchant") && (
              ...
              >
                ...
                <div className="text-left">
                  <p className="font-semibold text-[var(--color-text)]">{t("merchant")}</p>
                  <p className="text-xs text-[var(--color-text-muted)]">{t("merchantDesc")}</p>
                </div>
              </button>
            )}
            ...
            {availableRoles.includes("customer") && (
              ...
              >
                ...
                <div className="text-left">
                  <p className="font-semibold text-[var(--color-text)]">{t("customer")}</p>
                  <p className="text-xs text-[var(--color-text-muted)]">{t("customerDesc")}</p>
                </div>
              </button>
            )}
          </div>
      ...
      {step === "welcome" && (
        <div className="w-full max-w-xs space-y-4 animate-fade-in">
          <p className="text-sm text-[var(--color-text-muted)] text-center">{t("howToContinue")}</p>
      ...
      {step === "post_signout_role" && (
        <div className="w-full max-w-xs space-y-4 animate-fade-in">
          <p className="text-sm text-[var(--color-text-muted)] text-center">{t("chooseAccount")}</p>
          ...
          >
            ...
            <div className="text-left">
              <p className="font-semibold text-[var(--color-text)]">{t("continueAsMerchant")}</p>
              <p className="text-xs text-[var(--color-text-muted)]">{t("merchantDesc")}</p>
            </div>
          </button>
          ...
          >
            ...
            <div className="text-left">
              <p className="font-semibold text-[var(--color-text)]">{t("continueAsCustomer")}</p>
              <p className="text-xs text-[var(--color-text-muted)]">{t("customerDesc")}</p>
            </div>
          </button>
          ...
          <div className="relative my-2">
            ...
            <div className="relative flex justify-center text-xs">
              <span className="px-3 bg-[var(--color-bg)] text-[var(--color-text-muted)]">{t("or")}</span>
            </div>
          </div>
      ...
      {step === "phone" && (
        <div className="w-full max-w-xs space-y-4 animate-fade-in">
          <p className="text-sm text-[var(--color-text-muted)] text-center">
            {authMode === "register" ? t("enterPhoneRegister") : t("signInWithPhone")}
          </p>

          <div>
            <label className="text-sm font-medium text-[var(--color-text)]">{t("phoneNumber")}</label>
          ...
          {phoneErrorAction && (
            ...
            >
              {phoneErrorAction === "signin" ? t("signInInstead") : t("createNewInstead")}
            </button>
          )}
        ...
        <div className="w-full max-w-xs space-y-4 animate-fade-in">
          <div className="bg-[var(--color-primary)]/5 rounded-xl p-3 text-center">
            <p className="text-xs text-[var(--color-text-muted)]">
              {t("otpSentTo")} <span className="font-medium text-[var(--color-text)]">+977{phone}</span>
            </p>
          </div>

          <div>
            <label className="text-sm font-medium text-[var(--color-text)]">{t("enterOtp")}</label>
            <input
              ref={otpRef}
              type="tel"
              inputMode="numeric"
      ...
      {step === "forgot_phone" && (
        <div className="w-full max-w-xs space-y-4 animate-fade-in">
          <p className="text-sm text-[var(--color-text-muted)] text-center">{t("forgotPhoneDesc")}</p>
