  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-2xl font-bold text-[var(--a-text)] tracking-tight mb-1">Dispute Resolution</h1>
          <p className="text-sm text-[var(--a-muted)]">Review and resolve transaction disputes</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          {disputes.map((d) => (
            <div key={d.id} className="rounded-xl border border-[var(--a-border)] bg-[var(--a-surface-2)]/50 shadow-lg p-5">
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1.5">
                    <span className="font-semibold text-sm text-[var(--a-text)] truncate">{d.merchantName}</span>
                  ...
                  <p className="text-[10px] text-[var(--a-muted)] mt-2">{new Date(d.createdAt).toLocaleString()}</p>
                </div>
                <button
