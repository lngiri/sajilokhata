...
export default function AlertsPage() {
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold text-[var(--a-text)] tracking-tight mb-1">Anomaly Alerts</h1>
      <p className="text-sm text-[var(--a-muted)] mb-8">Suspicious activity flagged for review</p>
          ...
          {alerts.map((alert) => (
            <div key={alert.id} className={`rounded-xl p-5 border shadow-lg ${severityClass(alert.severity)}`}>
              <div className="flex items-center justify-between mb-1.5">
                <span className="font-semibold text-sm text-[var(--a-text)]">{alert.merchantName}</span>
                <span className={`text-[10px] uppercase font-semibold px-2 py-0.5 rounded-full border ${severityClass(alert.severity)}`}>
                  {alert.severity}
                </span>
              </div>
              <p className="text-sm text-[var(--a-text-2)]">{alert.message}</p>
              <p className="text-[10px] text-[var(--a-muted)] mt-2">{new Date(alert.createdAt).toLocaleString()}</p>
            </div>
          ))}
        </div>
      )}
