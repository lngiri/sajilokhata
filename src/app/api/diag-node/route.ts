...
export async function GET() {
  ...
  add("=== /api/diag-node (NODE runtime) ===");
  ...
  try {
    ...
    add("node self-sign->verify: " + (v ? "OK" : "FAILED"));
  } catch (e: any) {
    add("node self-sign threw: " + (e?.message ?? String(e)));
  }
  ...
  if (process.env.SUPABASE_SERVICE_ROLE_KEY && process.env.NEXT_PUBLIC_SUPABASE_URL) {
    try {
      ...
      await (admin.from("diag_logs") as any).insert({ runtime: "node", report: lines.join("\n") });
      add("DB self-report: INSERTED");
    } catch (e: any) {
      add("DB self-report FAILED: " + (e?.message ?? String(e)));
    }
  } else {
    add("DB self-report skipped: key/url missing on node");
  }
  ...
  if (raw && userId) {
    try {
      ...
      if (url && key) {
        ...
        add("role lookup skipped: url/key missing on node");
      }
    } catch (e: any) {
