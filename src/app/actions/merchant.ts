...
export async function getMerchantStats(merchantId: string): Promise<{
  ...
  const todayExpenses = expenseLogs.reduce((sum: number, l: any) => {
    if (!l.created_at?.startsWith(today)) return sum;
    return sum + l.amount;
  }, 0);
...
export async function getMerchantCustomerBalance(merchantId: string, customerId: string): Promise<{ balance: number; creditLimit: number }> {
  ...
  // Compute balance from approved credit logs (same logic as getMerchantCustomers)
  // Does not depend on the get_customer_balance RPC existing
...
export async function getMerchantDashboardData(merchantId: string) {
  ...
  // Total expenses (all time, for the expense card)
  const totalExpenses = expenseLogs.reduce((sum: number, l: any) => sum + l.amount, 0);

  // Today's expenses (subtracted from cash in hand)
  const todayExpenses = expenseLogs.reduce((sum: number, l: any) => {
    if (!l.created_at?.startsWith(today)) return sum;
    return sum + l.amount;
  }, 0);
