...
export default function RoleSwitcher({ compact }: RoleSwitcherProps) {
  ...
  if (compact) {
    return (
      <button
        ...
        disabled={switching}
        aria-label={t("switchTo", { role: t(isMerchant ? "customer" : "merchant") })}
        ...
        title={t("switchTo", { role: t(isMerchant ? "customer" : "merchant") })}
      >
        {switching ? (
  ...
  return (
    ...
    >
      ...
      <span>{isMerchant ? t("merchant") : t("customer")}</span>
      <svg className="w-3 h-3 text-slate-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
        <path strokeLinecap="round" strokeLinejoin="round" d="M7.5 21L3 16.5m0 0L7.5 12M3 16.5h13.5m0-13.5L21 7.5m0 0L16.5 12M21 7.5H7.5" />
      </svg>
      <span className="text-[10px] text-slate-400">{t("switch")}</span>
    </button>
  );
}
